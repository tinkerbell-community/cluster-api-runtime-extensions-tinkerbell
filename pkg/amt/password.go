...
// rules.
var ErrPasswordPolicy = errors.New("amt: password does not satisfy AMT complexity rules")
...
func (c *Client) SetAdminPassword(_ context.Context, username, realm, newPassword string) error {
	...
	if realm == "" {
		return errors.New("amt: digest realm is required to set a password")
	}
	...
	if err != nil {
		return fmt.Errorf("amt: setting admin password: %w", err)
	}
	if rv := resp.Body.SetAdminResponse.ReturnValue; rv != 0 {
		return fmt.Errorf("amt: setting admin password rejected with return value %d", rv)
	}
	return nil
}
...
func randomChar(alphabet string) (byte, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("amt: generating password: %w", err)
	}
...
func shuffle(b []byte) error {
	for i := len(b) - 1; i > 0; i-- {
		...
		if err != nil {
			return fmt.Errorf("amt: generating password: %w", err)
		}
